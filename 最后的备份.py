...
class AudioProcessor(QMainWindow):
    ...
    def setup_ui(self):
        ...
        btn_row = QHBoxLayout()
        self.extract_bg_btn = ModernButton("🎹 提取伴奏", "secondary")
        self.extract_bg_btn.clicked.connect(self.extract_bg)
        self.extract_bg_btn.setEnabled(False)
        btn_row.addWidget(self.extract_bg_btn)
        ...
        self.mode_combo.addItems(["原音频", "提取后人声", "提取后伴奏"])
        ...
        # Matplotlib图表
        self.fig, (self.ax_stft, self.ax_vocal_stft) = plt.subplots(2, 1, figsize=(6, 8))
        ...
        self.ax_stft.set_title('原始音频', color='white', fontsize=11)
        ...
        self.ax_vocal_stft.set_title('处理后音频', color='white', fontsize=11)
    ...
    def _load_audio_finished(self, result):
        ...
        self.extract_bg_btn.setEnabled(True)
    ...
    def extract_vocal(self):
        """提取人声"""
        self._extract_audio("vocal")
    
    def extract_bg(self):
        """提取伴奏"""
        self._extract_audio("bg")
    
    def _extract_audio(self, extract_type):
        """提取音频"""
        global vocal_data
    ...
    def _process_audio_finished(self, result):
        ...
        self.show_toast("音频提取完成！", "success")
        self.update_time_display()
    ...
    def _play_audio(self):
        ...
        while is_playing:
            ...
            mode = self.mode_combo.currentIndex()  # 0:原音频, 1:提取后人声, 2:提取后伴奏
            ...
            if play_position + CHUNK < len(audio_data):
                ...
                if mode == 0:  # 原音频
                    ...
                    if vocal_data is not None:
                        ...
                        current_vocal_chunk = audio_data[play_position:play_position+CHUNK]
                elif mode == 2:  # 提取后伴奏（需要计算伴奏）
                    if vocal_data is not None:
                        # 伴奏 = 原音频 - 人声
                        original = audio_data[play_position:play_position+CHUNK]
                        vocal = vocal_data[play_position:play_position+CHUNK]
                        current_vocal_chunk = original - vocal
                    else:
                        current_vocal_chunk = audio_data[play_position:play_position+CHUNK]
    ...
    def calc_stft(self):
        """计算STFT"""
        global current_stft_data, current_vocal_stft_data
